...
def get_kline_hour(currency: str, currency_begin_time: datetime, col_dict: dict, log_obj: Logger):
    table_name = f"kline_hour_{currency}"
    judage_loop = True
    while judage_loop:
        loop_begin = time()
        engine = mysql_engine()
        ...
        time_se = pd.date_range(start=currency_begin_time, periods=1993, freq="h").to_series()
        ...
        name, data = get_kline(f"gate-io_{currency.upper()}_USDT", currency_begin_int, currency_end_int, "1h")
