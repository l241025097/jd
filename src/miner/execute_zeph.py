...
from pool import (
    ...
    get_payout_csv,
    process_payout_csv,
    get_currency_price_from_sevenseas
)
...
def execute(log_obj):
    ...
    wallet_addr = "ZEPHYR2yZhaRugXYMSYM4rBMKENGEnnVJiArMLAe2o5A56iABzePDWZQGkH9BF3cXeA9EoMbsKXewFro5n2D5quqGNVRjy7rviW43"
    new_worker_hash_rate = 6000
    new_worker_power_w = 120
    ...
    currency_price_dict = get_currency_price_from_sevenseas(currency_pair)
    return_dict.update(currency_price_dict)
    log_obj.info(currency_price_dict)
    avg_currency_price = currency_price_dict["priceYesterday"]
...
if __name__ == "__main__":
    BEGIN = datetime.now()
    APP_NAME = f"zeph_earn_{BEGIN.strftime('%Y%m%d')}"
